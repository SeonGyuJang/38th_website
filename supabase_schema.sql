...
CREATE TABLE IF NOT EXISTS bus_trips (
    ...
    note TEXT,
    booking_override VARCHAR(10),  -- NULL(자동 마감일 기준), 'open'(관리자가 강제로 예약 오픈), 'closed'(관리자가 강제로 예약 잠금)
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE (trip_date, direction)
);

-- 이미 배포된 DB에 bus_trips가 location/booking_override 컬럼 없이 먼저 생성되어 있을 수 있으므로 추가 (재실행 안전)
ALTER TABLE bus_trips ADD COLUMN IF NOT EXISTS location VARCHAR(200);
ALTER TABLE bus_trips ADD COLUMN IF NOT EXISTS booking_override VARCHAR(10);
